...
# 18. 11. 2017
#
# selection of features for feature vectors for relative pronouns

import math

class Conll_rel_selector():    
    def process_node( self, node, doc):
        self.doc = doc
        self.feature_vectors = []
        if ( node.upos in [ "PRON", "DET" ] and "Rel" in node.feats[ "PronType" ] ):     
    ...
    def consider_candidate( self, node, candidate): # -> bool
        ...
        if ( node == candidate ):
            return False
        if ( candidate.upos in [ "NOUN", "PRON", "DET", "VERB" ] ):
            return True          
        return False
    def print_feature_vector( self, node, candidate): # void
        ...
        if ( parent != None ):
            ...
            if ( greatparent != None ):
                if ( greatparent == candidate ):
                    basic_relpron_relation = True
        feature_vector.append( basic_relpron_relation)
