# PRI NEVYJADRENYCH ZAMENACH ZAPISOVAT ID RIADIACEHO CLENU!
...
class PDT_get_coreference:    
    ...
    def read_node( self, first_line, supernode_ID): # -> string
        ...
        line = self.pdt_t_input.readline()
        perspron = False # if the t_lemma of this node is #PersPron - would be considered with dropped pronouns
        ...
        # controlling </LM> tag wouldn't work as there are inner <LM>...</LM> blocks
            if ( "<t_lemma>#PersPron" in line ):
                perspron = True
            elif ( "<coref_gram" in line or "<coref_text" in line ): # not coref special
                coref_record = self.process_coref( line, actual_infos, perspron)
    ...
    def ID_type( self, string): # -> string
        """
        recognizes type of the id: word, dropped word, sentence
        """
...
class Coreference_record:    
    def __init__( self, own_dropped, own_ID, coref_dropped, coref_ID, perspron):
        self.own_dropped = own_dropped # bools
        self.coref_dropped = coref_dropped
        # self.t_lemma
        self.own_ID = own_ID # strings
        self.coref_ID = coref_ID
        self.perspron = perspron # bool, if the record had t_lemma #PersPron - will be used, if the pronoun is dropped
